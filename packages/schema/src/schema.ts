...
export const CatholicChurchOrganizationSchema = z
	...
	.refine(
		(schema) => {
			const potentialActionRecipientIdMatches = schema.potentialAction?.recipient
				? schema.potentialAction?.recipient["@id"] === schema["@id"]
				: true

			const eventOrganizerIdMatches = schema.event?.[0]?.organizer
				? schema.event?.[0]?.organizer?.["@id"] === schema["@id"]
				: true

			const eventLocationPlaceType = schema.event?.[0]?.location?.find((loc) => loc["@type"] === "Place")
			const eventLocationAddressIdMatches = eventLocationPlaceType
				? eventLocationPlaceType.address["@id"] === schema.address?.["@id"]
				: true
			const idsMatch = potentialActionRecipientIdMatches && eventOrganizerIdMatches && eventLocationAddressIdMatches
...
export type CatholicChurchOrganizationSchema = z.infer<typeof CatholicChurchOrganizationSchema>
